...
description: >
  Generate a personalized 7-email onboarding sequence for Fluidz early access signups.
  ...
  for each email). Use this skill whenever someone signs up for Fluidz early access, when
...
Research a new Fluidz signup, then generate all 7 onboarding emails personalized with real context about their role and company. Output goes to a shared Notion database — one row per person, every email ready to copy-paste and send on schedule.
