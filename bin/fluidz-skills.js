...
/**
 * @edupegoretti/fluidz-skills CLI
 *
 * Install Claude Code skills from the @edupegoretti/fluidz-skills registry.
 *
 * Usage:
 *   npx github:edupegoretti/fluidz-skills install <slug> [--claude|--codex|--cursor] [--project-dir <path>]
 *   npx github:edupegoretti/fluidz-skills list             # List available skills
 *   npx github:edupegoretti/fluidz-skills info <slug>      # Show skill details
 */
...
const REPO = 'edupegoretti/fluidz-skills';
const BRANCH = 'main';
...
function fetch(url) {
  return new Promise((resolve, reject) => {
    const get = (u) => {
      https.get(u, { headers: { 'User-Agent': '@edupegoretti/fluidz-skills-cli' } }, (res) => {
...
async function installSkill(options) {
  ...
  if (!skill) {
    ...
    console.error(`Run "npx github:edupegoretti/fluidz-skills list" to see available skills.`);
    process.exit(1);
  }
...
async function listSkills() {
  ...
  console.log(`Install: npx github:edupegoretti/fluidz-skills install <slug>`);
}
...
async function showInfo(slug) {
  ...
  console.log(`\nInstall: npx github:edupegoretti/fluidz-skills install ${skill.slug}`);
...
switch (command) {
  case 'install':
    try {
      ...
      console.error(err.message);
      console.error('Usage: npx github:edupegoretti/fluidz-skills install <slug> [--claude|--codex|--cursor] [--project-dir <path>]');
      process.exit(1);
    }
    break;
  case 'list':
  ...
  case 'info':
    if (!args[0]) {
      console.error('Usage: npx github:edupegoretti/fluidz-skills info <slug>');
      process.exit(1);
    }
    showInfo(args[0]);
    break;
  default:
    console.log('@edupegoretti/fluidz-skills — GTM Skills para Claude Code by Fluidz\n');
    ...
    console.log('\nExamples:');
    console.log('  npx github:edupegoretti/fluidz-skills list');
    console.log('  npx github:edupegoretti/fluidz-skills install reddit-scraper');
    console.log('  npx github:edupegoretti/fluidz-skills install reddit-scraper --codex');
    console.log('  npx github:edupegoretti/fluidz-skills install reddit-scraper --cursor --project-dir /path/to/project');
    break;
}
