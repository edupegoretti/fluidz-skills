...
function parseInstallOptions(args) {
  if (!args[0] || args[0].startsWith('--')) {
    throw new Error('Usage: npx github:edupegoretti/fluidz-skills install <slug> [--claude|--codex|--cursor] [--project-dir <path>]');
  }
...
function renderCursorRule(slug, skillContent) {
  return [
    '---',
    `description: Fluidz skill instructions for ${slug}`,
    'alwaysApply: false',
    '---',
    '',
    'Always apply this rule when the user requests this Fluidz skill workflow.',
    '',
    skillContent.trim(),
    '',
  ].join('\n');
...
function placeForCursor(sourceSkillDir, projectDir) {
  ...
  const rulePath = path.join(rulesDir, `fluidz-${slug}.mdc`);
