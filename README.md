...
**Skills de GTM para [Claude Code](https://claude.ai/claude-code) — by [Fluidz](https://fluidz.com.br)**

Skills prontas para uso em vendas, marketing, inteligência competitiva, SEO e geração de leads.
...
```bash
npx github:edupegoretti/fluidz-skills install google-ad-scraper --claude
npx github:edupegoretti/fluidz-skills install google-ad-scraper --codex
npx github:edupegoretti/fluidz-skills install google-ad-scraper --cursor --project-dir /path/to/repo
```

- `--claude` (default): installs under `~/.claude/skills/<slug>/`
- `--codex`: installs under `~/.claude/skills/<slug>/`, then copies to `~/.codex/skills/<slug>/`
- `--cursor`: installs under `~/.claude/skills/<slug>/`, then writes a project-local Cursor rule file at `.cursor/rules/fluidz-<slug>.mdc`

Notes:
- Only one target flag is allowed per command.
- `--cursor` requires `--project-dir`.

## Available Skills (124)

**54 Capabilities** (atomic, single-purpose tools) | **61 Composites** (multi-skill chains) | **9 Playbooks** (end-to-end workflows)
...
```bash
npx github:edupegoretti/fluidz-skills list             # List all available skills
npx github:edupegoretti/fluidz-skills install <slug>   # Install for Claude Code (default)
npx github:edupegoretti/fluidz-skills install <slug> --codex
npx github:edupegoretti/fluidz-skills install <slug> --cursor --project-dir /path/to/repo
...
node bin/fluidz-skills.js list  # Test locally
```
